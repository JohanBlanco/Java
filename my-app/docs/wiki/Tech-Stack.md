...
Documentación del stack actual de **GymPlatform** (API, base de datos, web, móvil y herramientas). Pensada para onboarding, testing e improvements.
...
SaaS **multi-tenant** para gimnasios: un backend REST con JWT, un panel web React y una app Flutter; en desarrollo la DB es **H2 en archivo**, lista para **PostgreSQL** en producción.
...
| **Móvil** | Flutter | SDK **≥3.2** | — |
| | Provider + http + shared_preferences | estado + API + token | — |
...
| `http` | Llamadas REST |
| `provider` | Estado |
| `shared_preferences` | Token / preferencias |
| `intl` | Fechas / locale |

Paridad parcial con web (mismos endpoints JWT). Prioridad de mejoras suele ir primero en web.
...
| Tests | Spring Test + Security Test (backend); Flutter `flutter_test`; web sin suite formal aún |
